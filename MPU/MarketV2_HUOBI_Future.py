...
class MarketData_HUOBI_Future:
    def __init__(self, debug_mode: bool = True, redis_config: dict = None):
        ...
        self.__ws_url = "wss://www.hbdm.com/ws"
        self.__rest_depth_url = ""
        self.__symbol_book = {
                            "BTC_CQ": "",  # the exchange needs 'btcusdt'
                            "BTC_CW": "",
                            "BTC_NW": "",
                            "BTC_NQ": "",
                            "ETH_CQ": "",
                            ...
                            if 'ping' in msg:
                                ...
                                if 'trade' in msg['ch']:
                                    for trade in msg['tick']['data']:
                                        ...
                                        if "_CQ" in symbol:
                                            symbol = self.renameCQSymbol(symbol, dt)
                                        elif "_CW" in symbol:
                                            symbol = self.renameCWSymbol(symbol, dt)
                                        elif "_NW" in symbol:
                                            symbol = self.renameNWSymbol(symbol, dt)
                                        elif "_NQ" in symbol:
                                            symbol = self.renameNQSymbol(symbol, dt)
                                    ...
                                    data = msg['tick']
                                    symbol = (msg['ch'].split('.')[1]).upper()
                                    dt = datetime.utcfromtimestamp(data['ts'] / 1000)
                                    if "_CQ" in symbol:
                                        symbol = self.renameCQSymbol(symbol, dt)
                                    elif "_CW" in symbol:
                                        symbol = self.renameCWSymbol(symbol, dt)
