...
class MarketData_OKEX_Future:
    ...
    def load_valid_symbols(self):
        while True:
            try:
                rsp = requests.request(method="GET", url="https://www.okex.com/api/futures/v3/instruments", timeout=5)
                if rsp.status_code == 200:
                    ...
                    for instrument in instrument_list:
                        ...
                        if first != second and first != -1 and second != -1 and instrument[0:first] in ["BTC", "ETH"]:
                            symbols[instrument] = f'{instrument[0:first]}_{instrument[first+1:second]}@{instrument[second+1:]}'
            ...
            try:
                async with aiohttp.ClientSession() as ws_session:
                    async with ws_session.ws_connect(self.__ws_url, heartbeat=10, autoclose=True) as ws:
                        ...
                        depth_snaps = dict()
                        await ws.send_json({"op": "subscribe", "args": [f'futures/trade:{k}' for k, v in symbols.items()]})
                        await ws.send_json({"op": "subscribe", "args": [f'futures/depth_l2_tbt:{k}' for k, v in symbols.items()]})
                        ...
                        async for ws_msg in ws:
                            ...
                            if ws_msg.type == aiohttp.WSMsgType.BINARY:
                                ...
                                if "table" in msg:
                                    if msg["table"] == "futures/depth_l2_tbt":
                                        if "data" in msg:
                                        ...
                                        if "data" in msg:
                                            for data in msg["data"]:
                                                ...
                                                self.__publisher.pub_tradex(symbol=symbol,
                                                                            ...
                                                                            px_qty=(float(data["price"]), float(data["qty"])))
                ...
                while True:
                    try:
                        ...
                        for symbol in list(symbols.keys()):
                            async with session.get(url=f"https://www.okex.com/api/futures/v3/instruments/{symbol}/book",
